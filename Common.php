...
//Connection Settings @0-7B404956
$CCConnectionSettings = array (
    "Connection1" => array(
        "Type" => "MySQL",
        "DBLib" => "MySQL",
        "Database" => "codechargecalendar",
        "Host" => "localhost",
        "Port" => "3306",
        "User" => "root",
        "Password" => "",
...
//Initialize Common Variables @0-9CD43549
...
$CCSLocales->AddLocale("es", Array("es", "ES", array(1, 0, ""), 2, ",", ".", array("enero", "febrero", "marzo", "abril", "mayo", "junio", "julio", "agosto", "septiembre", "octubre", "noviembre", "diciembre"), array("ene", "feb", "mar", "abr", "may", "jun", "jul", "ago", "sep", "oct", "nov", "dic"), array("domingo", "lunes", "martes", "miércoles", "jueves", "viernes", "sábado"), array("dom", "lun", "mar", "mié", "jue", "vie", "sáb"), array("D", "L", "M", "M", "J", "V", "S"), array("dd", "/", "mm", "/", "yyyy"), array("dddd", ", ", "dd", " de ", "mmmm", " de ", "yyyy"), array("H", ":", "nn"), array("H", ":", "nn", ":", "ss"), "", "", 1, false, "", "utf-8", "UTF-8"));
...
define("CCS_ENCRYPTION_KEY_FOR_COOKIE", '164Z9IFQ533F2BL3');
define("CCS_EXPIRATION_DATE", 30 * 24 * 3600);
define("CCS_SLIDING_EXPIRATION", true);
...
//Initialize CacheManager @0-98DB223B
$CCSCacheManager = new clsCacheManager();
...
function CCSetALCookie($login, $password) {
    ...
    CCSetCookie("calendarLogin", $result, time() + CCS_EXPIRATION_DATE);
}
//End CCSetALCookie

//CCRefreshALCookie @0-665A6FEE
function CCRefreshALCookie($expirationDate) {
    if (CCS_SLIDING_EXPIRATION) {
        if (($expirationDate - (CCS_EXPIRATION_DATE / 2)) > time()) {
            list($login, $password, $expDate) = CCParseALCookie("calendarLogin");
...
//CCCheckAutoLoginCookies @0-046554D4
function CCCheckAutoLoginCookies() {
    if (!CCGetUserLogin()) {
        $parts = CCParseALCookie("calendarLogin");
...
//CCLoginUser @0-08C2A1E4
function CCLoginUser($login, $password)
{
    CCLogoutUser();
    $db = new clsDBConnection1();
    $SQL = "SELECT user_id, group_id, user_password FROM tbl_users WHERE user_login=" . $db->ToSQL($login, ccsText) . " AND user_password=MD5(" . $db->ToSQL($password, ccsText) . ")";
    $db->query($SQL);
