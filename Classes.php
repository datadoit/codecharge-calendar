...
//clsCacheManager class @0-1B045E38
class clsCacheManager {
  public $DataSource;
  public $Table = "cc_tbl_cache";
  public $KeyField = "cache_key";
  public $ExpiredField = "cache_expiration";
  public $DataField = "cache_data";


  public $Errors;

  function clsCacheManager() {
    $this->Errors = new clsErrors;
    $this->DataSource = new clsDBConnection1();
  }
  ...
  function PutObject($key, $object, $duration) {
    if ($this->Errors->count()) 
      return false;
    $expired = $duration ? time() + $duration : 0;
    @$this->DataSource->query("delete from " . $this->Table . " where " . $this->KeyField . "=" . $this->DataSource->ToSQL($key, ccsText));
    if (!$this->DataSource->Errors->count()) {
      @$this->DataSource->query(
        "insert into " . $this->Table . "(" . $this->KeyField . ", " . $this->ExpiredField . ", " . $this->DataField . ") values (" .
        $this->DataSource->ToSQL($key, ccsText) . ", " . $this->DataSource->ToSQL($expired, ccsInteger) . ", " . $this->DataSource->ToSQL($object, ccsText) . ")" );
    }
    if ($this->DataSource->Errors->count()) 
  ...
  function GetObject($key) {
    if ($this->Errors->count()) 
      return false;
    $now = time();
    @$this->DataSource->query("select * from " . $this->Table . " where " . $this->KeyField . "=" . $this->DataSource->ToSQL($key, ccsText) . " and (" . $this->ExpiredField . " = 0 or " . $this->ExpiredField . " > " . $now .")");
    if ($this->DataSource->Errors->Count()) {
      $this->Errors->AddErrors($this->DataSource->Errors);
      return false;
    }
    if ($this->DataSource->next_record()) {
      return $this->DataSource->f($this->DataField);
    } else {
      return false;
    }
  }
  ...
  function ClearStartedWith($PageKey) {
    if ($this->Errors->count()) 
      return false;
    @$this->DataSource->query("delete from " . $this->Table . " where " . $this->KeyField . " LIKE " . $this->DataSource->ToSQL($PageKey . "%", ccsText));    
    if ($this->DataSource->Errors->count()) 
      $this->Errors->AddErrors($this->DataSource->Errors);
  }
  ...
  function ClearExpired() {
    if ($this->Errors->count()) 
      return false;
    $now = time();
    @$this->DataSource->query("delete from " . $this->Table . " where " . $this->ExpiredField . " != 0 and " . $this->ExpiredField . " < " . $this->DataSource->ToSQL($now , ccsInteger));    
    if ($this->DataSource->Errors->count()) 
      $this->Errors->AddErrors($this->DataSource->Errors);
  }
  ...
  function Clear() {
    if ($this->Errors->count()) 
      return false;
    @$this->DataSource->query("delete from " . $this->Table);
    if ($this->DataSource->Errors->count()) 
      $this->Errors->AddErrors($this->DataSource->Errors);
  }
...
//End clsCacheManager class

//clsMainPage Class @0-6A33D5C8
class clsMainPage
{
